...
const HeaderComponent = {
  ...
  methods: {
    collapsedMenu() {
      ...
      vm.setCookie(
        vm.G.cookie.collapse.name,
        vm.G.menu.collapsed,
        vm.G.cookie.collapse.expire
      );
      vm.$emit("collapsed");
      setTimeout(() => {
        vm.$root.$emit("resize");
      ...
      setTimeout(() => {
        ...
        vm.menus.map(item => {
          if (
            item.url &&
            item.url.length > 0 &&
            vm.location.indexOf(item.url) !== -1
          ) {
        ...
        if (data.url && data.url.length > 0) {
          window.location.href =
           `${data.url}?${vm.G.id.mapping.app}=${vm.G.id.app}&${vm.G.id.mapping.pro}=${vm.G.id.pro}&${vm.G.id.mapping.soa}=${data["soaProjectId"]}`;
        }
      }, 100);
    }
  },
  created() {
    const vm = this;
    vm.location = window.location.href;
    vm.$api.get(
      vm.G.api.project,
      {
        method: "getProLink",
