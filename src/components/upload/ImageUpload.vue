...
<template>
  ...
  <div
    class="wrapper "
    @drop.prevent.stop
    :class="{dragging:flag_drag, compressed:compressList.length}"
  >
    <div
    ...
    >
      <template v-if="item.status === 'finished'">
        ...
        >
          ...
          <div class="cover">
            <div
              class="cover-item view"
              @click.stop="handleView(item)"
            ></div>
            <div
              class="cover-item delete"
    ...
    <span
      v-if="noteText&&noteText.length"
      class="noteText"
    >{{noteText}}</span>

    <div class="compressList-table-wrapper">
      <Table
        class="compressList-table"
        :columns="columns"
        :data="compressList"
    ...
    <Modal
      ...
      class-name="modal_cover wi-modal-custom"
      footer-hide
    >
      <img
        :src="imgShow"
...
export default {
  ...
  data() {
    const vm = this,
      upload =
        vm.config && vm.config.uploadServer
          ? vm.config.uploadServer
          : vm.G.files.server.upload,
      server =
        vm.config && vm.config.fileServer
          ? vm.config.fileServer
          : vm.G.files.server.download,
      action = "/upload/normal/batch-file";
    return {
      accept:
        this.config && this.config.accept ? this.config.accept : "image/*",
      ...
      //需要压缩的体积
      compressSize:
        this.config && this.config.compressSize
          ? this.config.compressSize
          : null,
      ...
      compressServer:
        this.config && this.config.compressServer
          ? this.config.compressServer
          : null,
      maxCompressedSize:
        this.config && this.config.maxCompressedSize
          ? this.config.maxCompressedSize
          : null,
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      ...
      if (
        check &&
        this.config &&
        ((null != this.config.width && null != this.config.height) ||
          null != this.config.ratio)
      ) {
          ...
          reader.onload = function(loaded) {
            ...
            img.onload = function() {
              ...
              if (null != vm.config.width && null != vm.config.height) {
                if (
                  width != Number(vm.config.width) ||
                  height != Number(vm.config.height)
                ) {
                  vm.alertError(
                    `请上传宽高为${vm.config.width}*${vm.config.height}的图片`
                  );
                  reject();
                }
              } else {
                ...
                if (vm.config.ratio.indexOf("*") !== -1) {
                  ...
                  if (0 !== limitWidth && 0 !== limitHeight) {
                    ...
                    if (limitRatio !== width / height) {
                      vm.alertError(
                        `请上传宽高比例为${limitWidth}*${limitHeight}的图片`
                      );
                      reject();
                    }
                  }
            ...
            res => {
              if (0 == res.ret.retCode) {
                ...
                if (res.data[0].fileSize > this.maxCompressedSize * 1024) {
                  this.alertError(
                    `图片压缩后大小是${this.getFileSize(
                      res.data[0].fileSize
                    )}, 超过最大限值${this.getFileSize(
                      this.maxCompressedSize * 1024
                    )}.`
                  );
                  this.remove(file);
                }
    ...
    handleMaxSize(file) {
      this.alertError(
        `图片大小超过最大限值${this.getFileSize(this.maxSize * 1024)}.`
      );
    },
    // 拖动相关
    ...
    setDefaultList(urls) {
      ...
      for (let i in urls) {
        defaultList.push({
          url:
            urls[i].indexOf("http") == -1 ? this.fileServer + urls[i] : urls[i],
          name: i,
          path: urls[i]
        });
      }
